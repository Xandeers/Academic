  ...
  <div class="min-h-screen bg-bg px-5 py-10">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="mb-12">
        ...
        <p class="text-muted">Mettez à jour les informations de votre événement</p>
      </div>
      ...
      <div class="bg-s1 border border-white/5 rounded-2xl p-8">
        ...
        <div v-if="currentStep === 4" class="space-y-6">
          ...
          <div class="bg-accent/10 border border-accent/20 rounded-xl p-4">
            <p class="text-sm text-[#E9EEEC]">
              En cliquant sur "Enregistrer", les modifications seront appliquées à votre événement.
            </p>
          </div>
        </div>
        ...
        <div v-if="saveError" class="mt-4 text-sm text-center p-3 rounded-lg border bg-red-500/10 text-red-400 border-red-500/25">
          {{ saveError }}
        </div>
      </div>
...
const categoryOptions = computed(() =>
  categoriesFromApi.value.length > 0
...
onMounted(async () => {
  ...
  // Load event from API
  try {
    const event = await $fetch<any>(`/api/events/${eventId}`, {
      headers: { Authorization: `Bearer ${auth.token}` },
    })
    // Parse begin_date into date + time
    const begin = event.begin_date ? new Date(event.begin_date) : null
    const end = event.end_date ? new Date(event.end_date) : null
    const toDateStr = (d: Date | null) => d ? d.toISOString().slice(0, 10) : ''
    const toTimeStr = (d: Date | null) => d ? d.toTimeString().slice(0, 5) : ''
    // Find category label
    const catLabel = event.category?.[0]?.name ?? event.category?.[0]?.label ?? 'Musique'
    form.value = {
      title: event.title ?? '',
      description: event.description ?? '',
      category: catLabel as EventCategory,
      date: toDateStr(begin),
      time: toTimeStr(begin),
...
const isStepValid = computed(() => {
  switch (currentStep.value) {
    ...
    case 2:
      return form.value.date && form.value.time && !errors.value.date && !errors.value.time
    case 3:
...
const saveEvent = async () => {
  ...
  try {
    ...
    saveError.value = err?.data?.detail ?? 'Erreur lors de la mise à jour de l\'événement'
